...
namespace Editor
{
    public class SheetToLocalizationSync : EditorWindow
    {
        ...
        [SerializeField] private bool deleteKeysNotInSheet; // Danger: removes entries not present in a sheet
        [SerializeField] private bool reportOnly; // Dry-run
        ...
        async System.Threading.Tasks.Task SyncNow()
        {
            try
            {
                ...
                {
                    var key = trimWhitespace ? header[i].Trim() : header[i];
                    headerIndex.TryAdd(key, i);
                }
                ...
                if (collection == null)
                {
                    ...
                    if (!reportOnly)
                    {
                        var folder = "Assets/Localization";
                        if (!AssetDatabase.IsValidFolder(folder))
                            AssetDatabase.CreateFolder("Assets", "Localization");

                        collection = LocalizationEditorSettings.CreateStringTableCollection(collectionName, folder);
                    }
                    else
                    {
                        collection =
                ...
                foreach (var lh in localeHeaders)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(lh)) {
                        Log($"⚠️ Skipping empty locale header.");
                        continue;
                    }
                    ...
                    if (table != null || reportOnly) continue;
